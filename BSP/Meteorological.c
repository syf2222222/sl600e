...
//	unsigned char time[6];
	unsigned char temp[2];		//temperature
	unsigned char hum;   	//humidity
	unsigned char winds0[2]; 	//Wind speed
	unsigned char windd0[2]; //wind direction
	unsigned char rain[2]; 		//
	unsigned char pre[2]; 		//pressure
	unsigned char sun[2]; 		//sunshine time
	unsigned char winds1[2]; 
	unsigned char windd1[2]; 
	unsigned char winds10[2]; 
	unsigned char windd10[2]; 
	unsigned char windm10[2]; 
};

uint8_t qx_data_buf[32] __attribute__((at(buf_qx)));

uint32_t last_qx_time;
int index_qx_inc(uint8_t *in);
uint8_t qx_empty=0;	
extern uint32_t systime32;
int gain_qx_data(void) //struct qx_data *qx_d
{
	char u2_sent[10];
...
//	OS_ERR      err;
	...
	buf=0;
	for(i=0;i<function_nub[0];i++)
	{
		if(function_buf[i]==0x25)
		{
			buf=1;
		}
	}
...
//			printf("\r\n转换数据 Get:");
			...
			{
				time_grow=(systime32-last_qx_time);
...
//						01		23	45		67				89	1011		1213			1415					1617						1819						2021						2223
	...
	//			memcpy(qx_d->pre,&Get[8],2);//空	
			...
			index_qx_inc(qx_data_buf);
			
		}
		else
		{
			qx_data_buf[0]=1; //////for test

			memcpy(&qx_data_buf[1],systime,6);

			for(i=0;i<29;i++) qx_data_buf[i+7]=0;
			index_qx_inc(qx_data_buf);
			if(qx_empty==0)
			{
				qx_empty=1;
