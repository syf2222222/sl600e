...
#define DM9000_PKT_MAX	1518  // IP 1500 + 以太网首部 14 +CRC 4
...
extern u8 http_client_recvbuf[2000];
#if 1
...
//	rxbyte=(u8)DM_CMD;			//DM9000->DATA;						//进行第二次读取 
//	if(rxbyte)										//接收到数据
	{
...
#if printf_test			
				tcp_ip_printf((u8*)p->payload,(p->len-4),0);
...
//	unsigned char s_mac[6];   //源地址  //球机 44,47,CC,54,E6,8D
//	uint16 type;     //协议类型 (0x0806表示ARP 协议？)
//};
...
//	if((datas[12]==0x08)&&(datas[13]==0x00))	//ipv4协议，改IP首部校验和  
//	{
//		printf("\r\nIP首部校验和："); printf("%x,",((uint16_t)datas[24]<<8)+datas[25]);
//		datas[24]=0;
//		datas[25]=0;
//		sum=check_sum(&datas[14],20); //计算IP首部校验和
//		datas[24]=((sum>>8) & 0x0ff);
//		datas[25]=sum;
//		printf("   改成："); printf("%x,",((uint16_t)datas[24]<<8)+datas[25]);
//		if((datas[12]==0x08)&&(datas[13]==0x00)&&(datas[23]==1)&&(len>50)) 
//		{	
//			printf("\r\nIMCP校验和："); printf("%x,",((uint16_t)datas[36]<<8)+datas[37]);
//			datas[36]=0;
//			datas[37]=0;
//			sum=check_sum(&datas[34],(len-34));
//			datas[36]=((sum>>8) & 0x0ff);
//			datas[37]=sum;
//			printf("  改成："); printf("%x,",((uint16_t)datas[36]<<8)+datas[37]);
//		}
//	}
